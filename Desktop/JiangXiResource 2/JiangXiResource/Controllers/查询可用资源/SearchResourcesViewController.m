...
- (void)searchResourcesPortViewBtnClick:(SearchResourcesPortViewBtnTag)btnTag{
    switch (btnTag) {
        case FreePortBtnTag:{
            NSLog(@"________------------>>>>>>>>空闲端口");
            NSString *urls = @"http://product.chuanwg.com:8080/chuanwg//webResources/sysimgUploadFiles/1473322442166.604618660.jpg";
            NSString *url_str=[NSString stringWithString:[urls stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];

            XhNetWorking *request = [XhNetWorking downloadRequestUrl:url_str tag:0];
